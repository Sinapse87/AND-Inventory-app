...
import com.example.android.inventoryapp.data.WineContract.WineEntry;
...
public class DetailsActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private void saveWine() {
        ...
        if (mWineCostView.getText().toString() != null &&
            ...
            cvCost = Integer.valueOf(mWineCostView.getText().toString().trim());
        }

        int cvPrice = 0;
        if (mWinePriceView.getText().toString() != null &&
                !mWinePriceView.getText().toString().isEmpty()) {
            cvPrice = Integer.valueOf(mWinePriceView.getText().toString().trim());
        }
